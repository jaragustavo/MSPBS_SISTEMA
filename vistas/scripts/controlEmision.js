...
function listar()
{
        tabla=$('#tbllistado').dataTable(
        {
         ...
         "ajax":
           {
            url: '../ajax/controlEmision.php?op=listar',
            type : "get",
            dataType : "json",      
            error: function(e){
...
function listarOC()
{
    tabla=$('#tblOC').dataTable(
    {
        ...
        "ajax":
                {
                    url: '../ajax/controlEmision.php?op=listarOC',
                    type : "get",
                    dataType : "json",                      
                    error: function(e){
                        console.log(e.responseText);    
...
function guardaryeditar(e)
{
  ...
  //  $('#fecha_hora').prop('disabled',false);
    ...
    if( $('#anular').prop('checked') ) {
           var idcodigo = $('#codigo_control_emision').val();
           anular(idcodigo);
  
        
     }else {
          $('#codigo_control_emision').prop('disabled',false);
    ...
    $.ajax({
        url: "../ajax/controlEmision.php?op=guardaryeditar",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
  ...
  // $('#fecha_hora').prop('disabled',true);
    $('#codigo_control_emision').prop('disabled',true);
    }
...
function anular(idcodigo)
{
        ...
        if(result)
        {
            $.post("../ajax/controlEmision.php?op=anular", {idcodigo : idcodigo}, function(e){
                bootbox.alert(e);
                tabla.ajax.reload();
                mostrarform(false);
                limpiar();
...
function editarPlanillaEmision(codigo_planilla_emision)
{
        ...
        $.post("../ajax/controlEmision.php?op=mostrarEditar",{idcodigo : codigo_planilla_emision}, function(data, status)
        {
               data = JSON.parse(data);        
               mostrarform(true);
               $("#codigo_control_emision").val(data[0]);
        ...
        $.post("../ajax/controlEmision.php?op=listarDetalle&id="+codigo_planilla_emision,function(r){
                $("#detalles").html(r);
        });
...
//alert('Monto '+monto);
    if (codigo_orden_compra!="")
    {
        ...
        '<td><name="monto">'+monto+'</td>'+
         '<td><input type="text" name="obs[]"        id="obs[]"          value="'+obs+'"></td>'+
  ...
  // codigo = $("#codigo").val(); 
//   alert('codigo '+codigo);
   window.open('../ajax/imprimir/imprimirOC.php?codigo='+codigo);
