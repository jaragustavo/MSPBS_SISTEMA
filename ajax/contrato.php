...
$numero_expediente = isset($_POST["nro_expediente"]) ? limpiarCadena($_POST["nro_expediente"]) : "";
...
//error_log('#################################### '.$fecha_pedido);
//$idusuario=1; 

 function CargarArchivoCsv($detalles){    
    ...
    //    $rspta = $persona->listarPersona($val['numero_cedula']);
        ...
        echo '<thead style="background-color:#A9D0F5"  scope="col">
          ...
          foreach($detalles as $key => $val) {
            ...
            // error_log ('######### '.$key.' NumeroCedula: '.$val['numero_cedula'].' Item: '.$val['item'].' Monto: '.$val['monto']);
             $rspta = $persona->listarPersona($val['numero_cedula']);
              $reg = pg_fetch_array($rspta);
        
      //  while ($reg = pg_fetch_array($rspta)) {
            $cont = $cont + 1;
            
            $cantidad_vinculo = $contrato->contarVinculo($val['numero_cedula']);
    ...
    //        error_log('##### '.$cantidad_vinculo['cantidad_vinculo']);
            ...
            $cantidad_especialidad = $contrato->contarEspecialidad($val['numero_cedula']);
    ...
    //        error_log('##### '.$cantidad_vinculo['cantidad_vinculo']);
            ...
            while($row_cargo = pg_fetch_array($cargos))
              {
               $sw= $row_cargo['codigo']== $val['codigo_funcion'] ? 'selected':'';   
              // $sw='';
            ...
            while($row_dependencias = pg_fetch_array($dependencias))
              {
               $sw= $row_dependencias['codigo']== $val['codigo_ubicacion'] ? 'selected':'';   
            //   $sw='';
            //  error_log($row_cargo['codigo'].' '. $reg['cargo_funcion'].' '.$sw);
            ...
            while($row_og = pg_fetch_array($objeto_gasto))
              {
              $sw= $row_og['codigo']== $val['objeto_gasto'] ? 'selected':'';   
            ...
            while($row_region = pg_fetch_array($region_sanitaria))
             ...
             // $sw= $row_region['codigo']== $reg['codigo_region_sanitaria']? 'selected':'';   
              $sw='';
            //  error_log($row_cargo['codigo'].' '. $reg['cargo_funcion'].' '.$sw);
              $selectRegionSanitaria = $selectRegionSanitaria . '<option title="' . $row_region['nombre'] . '" value="' . $row_region['codigo'] . '" '.$sw.'>' . $row_region['codigo'] . '</option>';
            }
             ...
             $estado = '<option value="-1"> </option><option selected value="1">INCLUIR</option><option value="2">EXCLUIR</option>';
               ...
               echo '<tr  scope="col" id="fila'.$val['item']. '" class="filas" style="font-size: 12px;" >
       <td scope="col" style="text-align: center;"><div style="width: 15px;"><a class="btn btn-accent m-btn m-btn--custom
       m-btn--icon m-btn--air m-btn--pill" type="button" onclick="eliminarDetalle('.$val['item'].')" style="padding: unset;">
       <span><i class="fa fa-trash" style="color: indianred; "></i></span></a>
        <td scope="col"><input type="hidden" readonly="readonly" name="item[]" id="item[]" value="'.$val['item'].'">'.$val['item'].'</td>
      <td scope="col"><input type="hidden" readonly="readonly" name="numero_documento[]" id="numero_documento[]" value="'.$val['numero_cedula'].'">'.$val['numero_cedula'].'</td>
...
<td scope="col"><input type="hidden" readonly="readonly" name="vin[]" id="vin[]" value="'. $vinculo.'">'.$vinculo.'
       <a data-toggle="modal" href="#modalVinculo" class="btn btn-accent m-btn m-btn--custom 
                m-btn--icon m-btn--air m-btn--pill" type="button" title="Ver Vinculos" style="width:20%; font-size:14px;" onclick="verVinculo(' . $val['numero_cedula']. ')">
      ...
      <td scope="col" style="width: 100px;font-size: 100%;"><div style="width: 100px;font-size:100%;"><input style="width: 100px;font-size:100%;" type="text" name="salario[]" id="salario[]" value="' .number_format($val['monto'] ,0,'','.'). '"></div></td>
...
switch ($_GET["op"]) {
                                   ...
                                   //     $fecha_fin,
                                    ...
                                    //  $_POST["vin2"],
                                      ...
                                      //  $_POST["obs"],
                                        ...
                                        $fecha_pedido,
                                        $idusuario,
                                        $_POST["cic_reemplazante"],
                                        $_POST["nombre_reemplazante"]
                                        );
    ...
    case 'mostrar':
        $idcodigo = $_POST['idcodigo'];
        //  error_log('##########2344444 '.$idpedido);
        $rspta = $contrato->mostrar($idcodigo);
        $rspta = pg_fetch_array($rspta);
 ...
 case 'mostrarDetalle':
         $idcodigo = $_GET['id'];
         $nrows = $_GET['nrows'];
         $codigo_tipo_resolucion_concepto = $_GET['codigo_tipo_resolucion_concepto'];
      //error_log('# '. $codigo_tipo_resolucion_concepto);   
          $rspta = $contrato->mostrarDetalle($idcodigo);
           if($nrows == 0 and $codigo_tipo_resolucion_concepto != 16)  {
                echo '<thead style="background-color:#A9D0F5"  scope="col">
                                          <th scope="col"><div style="width: 15px;text-align: center; font-size: 80%;"></div></th>
                                          <th scope="col"><div style="width: 20px;text-align: center; font-size: 80%;">Item</div></th>
                                          <th scope="col"><div style="width: 50px;text-align: center; font-size: 80%;">Nro.Doc.</div></th>
                                        ...
                                        <th scope="col"><div style="width: 100px;text-align: center; font-size: 80%;">Función</div></th>
                                        <th scope="col"><div style="width: 60px;text-align: center; font-size: 80%;">Espec.</div></th>
                                         <th scope="col"><div style="width: 100px;text-align: center; font-size: 80%;">Region</div></th>
                                          <th scope="col"><div style="width: 50px;text-align: center; font-size: 80%;">Depen.</div></th>
                                         <th scope="col"><div style="width: 100px;text-align: center; font-size: 80%;">Estado</div></th> 
               ...
               $nrows =1;
            } else if ($nrows == 0 and $codigo_tipo_resolucion_concepto == 16 ) {
                 ...
                 echo '<thead style="background-color:#A9D0F5"  scope="col">
                                  ...
                                  <th scope="col"><div style="width: 60px;text-align: center; font-size: 80%;">Espec.</div></th>
                                   <th scope="col"><div style="width: 100px;text-align: center; font-size: 80%;">Region</div></th>
                                    <th scope="col"><div style="width: 50px;text-align: center; font-size: 80%;">Depen.</div></th>
                                   <th scope="col"><div style="width: 100px;text-align: center; font-size: 80%;">Estado</div></th> 
                                   <th scope="col"><div style="width:50px;text-align: center; font-size: 80%;">Cic.Reemp.</div></th>
                                   <th scope="col"><div style="width: 100px;text-align: center; font-size: 80%;">Reemplazante</div></th>   
                               <!--     
        ...
        if($codigo_tipo_resolucion_concepto != 16){
                while ($reg = pg_fetch_array($rspta)) {
                  ...
                  $cantidad_vinculo = $contrato->contarVinculo($reg['numero_documento']);
                  $cantidad_vinculo = pg_fetch_array($cantidad_vinculo);
          //        error_log('##### '.$cantidad_vinculo['cantidad_vinculo']);
                  $vinculo = $cantidad_vinculo['cantidad_vinculo'];   

                  $cantidad_especialidad = $contrato->contarEspecialidad($reg['numero_documento']);
                  $cantidad_especialidad = pg_fetch_array($cantidad_especialidad);
          //        error_log('##### '.$cantidad_vinculo['cantidad_vinculo']);
                  $especialidad = $cantidad_especialidad['cantidad_especialidad'];
                  ...
                  if($reg['codigo_estado']== '1'){
                      ...
                      echo '<tr  scope="col" id="fila'.$reg['numero_item']. '" class="filas" style="font-size: 12px;" >
             <td scope="col" style="text-align: center;"><div style="width: 15px;"><a class="btn btn-accent m-btn m-btn--custom
             m-btn--icon m-btn--air m-btn--pill" type="button" onclick="eliminarDetalle('.$reg['numero_item'].')" style="padding: unset;">
             <span><i class="fa fa-trash" style="color: indianred; "></i></span></a>
              <td scope="col"><input type="hidden" readonly="readonly" name="item[]" id="item[]" value="'.$reg['numero_item'].'">'.$reg['numero_item'].'</td>
           ...
           <td><select id="objeto[]" name="objeto[]">'.$selectOG.'</select></td>     
            <td scope="col"><input type="hidden" readonly="readonly" name="vin[]" id="vin[]" value="'. $vinculo.'">'.$vinculo.'
             <a data-toggle="modal" href="#modalVinculo" class="btn btn-accent m-btn m-btn--custom 
                      m-btn--icon m-btn--air m-btn--pill" type="button" title="Ver Vinculos" style="width:20%; font-size:14px;" onclick="verVinculo(' . $reg['numero_documento']. ')">
                      <span><i class="fa fa-eye" style="color: #82b74b;"></i></span></a>    

            </td>    

            <td scope="col" style="width: 45px;font-size: 100%;"><div style="width: 45px;font-size:100%;"><input style="width: 45px;font-size:100%;" type="text" name="carga_horaria[]" id="carga_horaria[]" value="' .$reg['carga_horaria']. '"></div></td>

            <td scope="col" style="width: 100px;font-size: 100%;"><div style="width: 100px;font-size:100%;"><input style="width: 100px;font-size:100%;" type="text" name="salario[]" id="salario[]" value="' .number_format($reg['salario'] ,0,'','.'). '"></div></td>

            <td><select id="cargo_funcion[]" name="cargo_funcion[]">'.$selectfuncion.'</select></td>  

            <td scope="col"><input type="hidden" readonly="readonly" name="cantidad_especialidad[]" id="cantidad_especialidad[]" value="'. $especialidad.'">'.$especialidad.'
             <a data-toggle="modal" href="#modalEspecialidad" class="btn btn-accent m-btn m-btn--custom 
                      m-btn--icon m-btn--air m-btn--pill" type="button" title="Ver Especialidad" style="width:20%; font-size:14px;" onclick="verEspecialidad(' . $reg['numero_documento'] . ')">
                      <span><i class="fa fa-eye" style="color: #82b74b;"></i></span></a>    
        ...
        } else if ($codigo_tipo_resolucion_concepto == 16) {
                          while ($reg = pg_fetch_array($rspta)) {
          ...
          //        error_log('##### '.$cantidad_vinculo['cantidad_vinculo']);
           ...
           <td><select id="objeto[]" name="objeto[]">'.$selectOG.'</select></td>     
            ...
            <td scope="col"><input type="hidden" readonly="readonly" name="cic_reemplazante[]" id="cic_reemplazante[]" value="'.$reg['cic_reemplazante'].'">'.$reg['cic_reemplazante'].'</td>
           
           <td scope="col" ><input type="hidden" readonly="readonly" name="nombre_reemplazante[]" id="nombre_reemplazante[]" value="'.$reg['nombre_reemplazante'].'">'.$reg['nombre_reemplazante'].'</td>
             
             </tr>';
                      //error_log($reg['nombre_reemplazante']);
         }           
       } 
  ...
  case 'agregarPersona':
        $cedula_identidad = $_GET['id'];
        $nrows = $_GET['nrows'];
        $codigo_tipo_resolucion_concepto = $_GET['codigo_tipo_resolucion_concepto'];
        $cic_reemplazante = $_GET['cic_reemplazante'];
         // error_log('# '.$codigo_tipo_resolucion_concepto);
       //   error_log('# '.$nrows);
          
        require_once "../modelos/Persona.php";
        $persona = new Persona();
        $rspta = $persona->listarPersona($cedula_identidad);
        $reemplazante = $persona->listarPersona($cic_reemplazante);
        $reg_reemplazante = pg_fetch_array($reemplazante);
        if($nrows == 0 and $codigo_tipo_resolucion_concepto != 16)  {
             echo '<thead style="background-color:#A9D0F5"  scope="col">
                   <th scope="col"><div style="width: 15px;text-align: center; font-size: 80%;"></div></th>
                    <th scope="col"><div style="width: 20px;text-align: center; font-size: 80%;">Item</div></th>
        ...
        if($codigo_tipo_resolucion_concepto != 16){
            ...
            //        error_log('##### '.$cantidad_vinculo['cantidad_vinculo']);
                   ...
                   // error_log($selectOG);
                    ...
                    //error_log($selectRegionSanitaria);
               //     if($reg['codigo_estado']== '1'){
                      $estado = '<option value="-1"> </option><option selected value="1">INCLUIR</option><option value="2">EXCLUIR</option>';
                //        
                //    }else {
                     //   $estado = '<option value="-1"> </option><option value="1">INCLUIR</option><option selected value="2">EXCLUIR</option>';
                  //  }         
                echo '  <tr  scope="col" id="fila'.$nrows. '" class="filas" style="font-size: 12px;" >
               <td scope="col" style="text-align: center;"><div style="width: 15px;"><a class="btn btn-accent m-btn m-btn--custom
               m-btn--icon m-btn--air m-btn--pill" type="button" onclick="eliminarDetalle('.$nrows.')" style="padding: unset;">
               <span><i class="fa fa-trash" style="color: indianred; "></i></span></a>
                <td scope="col"><input type="hidden" readonly="readonly" name="item[]" id="item[]" value="'.$nrows.'">'.$nrows.'</td>
             ...
             <td scope="col"><input type="hidden" readonly="readonly" name="vin[]" id="vin[]" value="'. $vinculo.'">'.$vinculo.'
              ...
              <td scope="col" style="width: 100px;font-size: 100%;"><div style="width: 100px;font-size:100%;"><input style="width: 100px;font-size:100%;" type="text" name="salario[]" id="salario[]" value="' .number_format(0 ,0,'','.'). '"></div></td>
             ...
             <td scope="col"><input type="hidden" readonly="readonly" name="vin[]" id="vin[]" value="'. $vinculo.'">'.$vinculo.'
              ...
              <td scope="col"><input type="hidden" readonly="readonly" name="cantidad_especialidad[]" id="cantidad_especialidad[]" value="'. $especialidad.'">'.$especialidad.'
               ...
               <td><select  id="codigo_ubicacion_prestacion[]" name="codigo_ubicacion_prestacion[]">'.$selectDependencias.'</select></td>
                ...
                <td scope="col"><input type="hidden" readonly="readonly" name="cic_reemplazante[]" id="cic_reemplazante[]" value="'.$reg_reemplazante['cedula_identidad'].'">'.$reg_reemplazante['cedula_identidad'].'</td>
              <td scope="col" style="width: 180px;font-size: 100%;"><div style="width: 180px;font-size:100%;"><input style="width: 180px;font-size:100%;" type="hidden" name="nombre_reemplazante[]" id="nombre_reemplazante[]" value ="'. $reg_reemplazante['nombres'].' '. $reg_reemplazante['apellidos'].'">'. $reg_reemplazante['nombres'].' '. $reg_reemplazante['apellidos'].'</div></td>
               </tr>'; 
             }
    ...
    case 'listar':
        ...
        while ($reg = pg_fetch_array($rspta)) {
           //  error_log('## '.$reg['codigo_lugar_envio']);
               ...
               '<a class="btn btn-accent m-btn m-btn--custom 
                m-btn--icon m-btn--air m-btn--pill" type="button" title="Ver Pedido" style="width:20%; font-size:18px;" onclick="verContrato(' . $reg['codigo'] . ')">
       ...
       //error_log('############entre ');
        ...
        if ($destinatario == "-1"){
            echo('Ingrese datos Estado Envio');
              ...
              echo $rspta ? "Expediente enviado" : "No se pudo enviar el expediente";
