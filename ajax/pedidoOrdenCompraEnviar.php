...
require_once "../modelos/PedidoOrdenCompraEnviar.php";

$pedidoOC=new PedidoOrdenCompra();

$pedidoOrdenCompraEnviar=new PedidoOrdenCompraEnviar();
...
switch ($_GET["op"]){
    case 'enviar':
        ...
        if ($idusuarioDestino == "-1" || $estadoCierre == "-1"){
                       echo('Ingrese todos los datos del formulario para envio');
        }
        else {
           
               $idcodigo =$_POST["idcodigo"];
               $idpedido = $_POST["idpedido"];
               $codigoMedicamento = $_POST["codigo_medicamento"];
             //  error_log('########### '.$codigoMedicamento); 
              // error_log('########### '.$idcodigo);
              // error_log('########### '.$idpedido); 
              $rspta=$pedidoOrdenCompraEnviar->enviar($idcodigo,$idpedido,$codigoMedicamento,$fechaPedido,$idusuario,null,$idusuarioDestino,$obs,$estadoCierre);
    ...
    case 'mostrarEnviar':
   // $idcodigo=isset($_POST["idcodigo"])? limpiarCadena($_POST["idcodigo"]):"";
        $rspta=$pedidoOrdenCompraEnviar->mostrarEnviar($_POST["idcodigo"]);
    ...
    case 'listarDetalle':
        ...
        $rspta = $pedidoOrdenCompraEnviar->listarDetalle($id);
        $total=0;
        ...
        while($reg = pg_fetch_row($rspta))
                {
                    ...
                    echo '<tr class="filas"><td><input name="idcodigo[]" id="idcodigo[]" type="hidden" value="'.$reg[0].'"> '.$reg[0].'</td><td>'.$reg[1].'</td><td><input name="idpedido[]" id="idpedido[]" type="hidden" value="'.$reg[9].'"> '.$reg[9].'</td><td><input name="codigo_medicamento[]" id="codigo_medicamento[]" type="hidden" value="'.$reg[4].'"> '.$reg[4].'</td><td>'.$reg[5].'</td><td>'.$reg[6].'</td><td>'.$reg[3].'</td><td>'.$reg[7].'</td></tr>';
    ...
    case 'listar':
        ...
        $rspta=$pedidoOrdenCompraEnviar->listar($idusuario);
        //Vamos a declarar un array
        $data= Array();
        while($reg = pg_fetch_row($rspta)){
            $data[]=array(
               "0"=>($reg[9]==2?'<input name="idcodigo" id="idcodigo" type="hidden" value="'.$reg[0].'">
