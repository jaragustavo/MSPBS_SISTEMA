...
Class Usuario
{
    ...
    //Implementamos un método para editar registros
       public function editar($idusuario,$nombre,$cedula_identidad,$login,$clave,$permisos)
 //   public function editar($idusuario,$nombre,$cedula_identidad,$login,$clave)
    ...
    {
        $sql="SELECT idusuario,cedula_identidad,nombre,login FROM usuario WHERE login='$login' AND clave='$clave' AND estado='AC'"; 
       
        return ejecutarConsulta($sql);  
    }
     public function selectUsuario()
    {
        $sql="SELECT identificador as idusuario,trim(apellidos)||' '||trim(nombres) as nombre FROM usuario  
              where id_estado_usuario = 2
              order by nombre;";
        return ejecutarConsultaSimpleFila($sql);
    }
}
