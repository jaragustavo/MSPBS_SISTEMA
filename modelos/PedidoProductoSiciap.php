...
//error_log('############AGREGAR'.$sql);
    ...
    public function anularEnvioPedido($idpedido)
    {
        $sql="UPDATE pedido_producto SET indicador_estado ='PENDIENTE' WHERE codigo=$idpedido[0]";
      //  error_log('%%%%%%%%%%%%%%% '.$sql);
        return ejecutarConsulta($sql);
    }
    ...
    {
        $sql="select ocp.codigo,numero_expediente,to_char (fecha_pedido, 'DD-MM-YYYY') as fecha_pedido,codigo_medicamento,(select clasificacion_medicamento||' '||coalesce(concentracion,'')||' '||forma_farmaceutica||' '||presentacion from descripcion_medicamento 
...
and   a.codigo =$idcodigo";
    ...
    //error_log('##########33 '.$sql);
      ...
      public function mostrarAnularEnvioPedido($idcodigo)
    {
...
and   a.codigo =$idcodigo";
 ...
 //  error_log('##########33 '.$sql);
    ...
    {
        $sql="SELECT numero_pedido,to_char (fecha_pedido, 'DD-MM-YYYY') as fecha_pedido ,pedido.codigo_medicamento,pedido.producto,pedido.cantidad,nombre_sucursal,
       stock,codigo_pedido_producto_detalle
FROM (
select    a.codigo as numero_pedido,b.codigo as codigo_pedido_producto_detalle,
           a.fecha_pedido,
           b.codigo_medicamento,
          (select clasificacion_medicamento||' '||coalesce(concentracion,'')||' '||forma_farmaceutica||' '||presentacion 
            from descripcion_medicamento 
...
inner join
(
select codigo_medicamento, producto, sum(cantidad) as stock from
...
group by codigo_medicamento, producto) as stock
ON pedido.codigo_medicamento = stock.codigo_medicamento
order by codigo_pedido_producto_detalle";
...
from pedido_producto a,pedido_producto_detalle b, sucursal c
where a.codigo = b.codigo_pedido
and   a.codigo_sucursal = c.codigo and a.idusuario ='$idusuario'";
        return ejecutarConsulta($sql);      
    }
