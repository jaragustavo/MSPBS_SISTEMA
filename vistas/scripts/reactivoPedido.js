...
function verMovimiento(idcodigo)
{
   ...
   $.post("../ajax/reactivoPedido.php?op=verMovimiento&id="+idcodigo,function(r){
...
function listar()
{
        tabla=$('#tbllistado').dataTable(
        {
         ...
         "ajax":
           {
            url: '../ajax/reactivoPedido.php?op=listar',
            type : "get",
            dataType : "json",      
            error: function(e){
...
function listarArticulos()
{
   ...
   for (var i = 0; i < idarticulo.length; i++) {
         var detalle = {
            "medicamento" : idarticulo[i].value
         };
    ...
    var medicamentoJSON = JSON.stringify(send);
        ...
        "ajax":
                {
                    ...
                    url: "../ajax/reactivoPedido.php?op=listarArticulos&id="+medicamentoJSON,
                    type: "POST",
                    contentType: false,
                    processData: false,           
                    error: function(e){
...
function mostrar(idcodigo)
{
    ...
    $.post("../ajax/reactivoPedido.php?op=mostrar",{idpedido : idcodigo}, function(data, status)
    {
       ...
       // $('#idcodigo').prop('disabled',true);
        ...
        $('#codigo_sucursal').selectpicker('refresh');
        $("#codigo_tipo_pedido").val(data['codigo_tipo_pedido']);
        $('#codigo_tipo_pedido').selectpicker('refresh');
...
function agregarDetalle(idarticulo,articulo,codigo_catalogo,especificacion_tecnica,presentacion)
  {
      //alert("hola "+ idarticulo);
        var btn_1 = document.getElementById('btnAgregar'+idarticulo);
        var btn_2 = document.getElementById('btnAgregado'+idarticulo);
         //alert("hola "+ idarticulo);
       // alert("hola "+ btn_2);
 ...
 //       $("#btnAgregar" + idarticulo).display = 'none';
   //     $("#btnOK" + idarticulo).display = 'inline';  
    if (idarticulo!="")
    {
       ...
       '<td scope="col"><div style="width: 60px; font-size: 80%;"><input onkeyup="formatNumero(this)" onchange="formatNumero(this)" type="text" size="9" name="precio_referencial[]" id="precio_referencial[]" value=""></div></td>'+
       '<td scope="col"><div style="width: 50px; font-size: 80%;"><input onkeyup="formatNumero(this)" onchange="formatNumero(this)" type="text" size="7" name="cantidad[]" id="cantidad[]"   value=""></div></td>'+
