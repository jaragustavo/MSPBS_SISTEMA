...
require_once "../modelos/PedidoOrdenCompraConsultarAnular.php";
 
$pedidoOC=new PedidoOrdenCompra();
$pedidoOrdenCompraRecibir=new PedidoOrdenCompraRecibir();
$pedidoOrdenCompraConsultarAnular=new PedidoOrdenCompraConsultarAnular();
...
switch ($_GET["op"]){
    ...
    case 'listar':
        ...
        $rspta=$pedidoOrdenCompraConsultarAnular->listar($idusuario);
        //Vamos a declarar un array
        $data= Array();
        while($reg = pg_fetch_row($rspta)){
            $data[]=array(
               "0"=>($reg[9]==3?'<input name="idcodigo" id="idcodigo" type="hidden" value="'.$reg[0].'">
               <input type="checkbox" name="chcodigo" id="chcodigo"  value="'.$reg[0].'"> ':
                     ...
                     ($reg[9]==1?'<button class="btn btn-primary btn" onclick="mostrarMovimiento('.$reg[1].','.$reg[5].',\''.$reg[6].'\')"><i class="fa fa-eye"></i></button>':  
                     ' '))),
